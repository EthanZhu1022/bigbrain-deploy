    ...
    const poll = async () => {
      try {
        const res = await axios.get(
          `https://bigbrain-backend-qff3.onrender.com/play/${playerId}/question`
        );
  ...
  const handleSubmitAnswer = async () => {
    ...
    try {
      const res = await axios.put(
        `https://bigbrain-backend-qff3.onrender.com/play/${playerId}/answer`,
        {
          answerIds,
        }
      );
    ...
    <Container className="mt-4">
      <Card className="mb-4">
        <Card.Body>
          ...
          {currentQuestion.media &&
            currentQuestion.media.startsWith("http") &&
      ...
      {gameEnded && (
        <Modal show onHide={() => navigate("/")}>
          ...
          <Modal.Body>
            <p>
              <strong>Scoring System Explanation:</strong>
            </p>
            <ul>
              <li>Each question&apos;s score = Question points × Speed factor</li>
              <li>Speed factor = Remaining time / Total time, minimum 0.1</li>
            ...
            <h5>
              ...
              {performance.reduce((acc, entry) => {
                const correct = entry.correctAnswerIds.every((id) =>
                  entry.selected.includes(id)
                );
                const speedFactor = Math.max(
                  0.1,
                  (entry.totalTime - entry.timeSpent) / entry.totalTime
                );
                const score = correct
                  ? Math.round(entry.points * speedFactor)
                  : 0;
                return acc + score;
              }, 0)}{" "}
              points
            ...
            <ul>
              {performance.map((entry, i) => {
                ...
                return (
                  <li key={i}>
                    <strong>Question {i + 1}:</strong> {entry.question}
                    <br />
